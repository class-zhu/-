...
def draw_error(dataset, errs):
    ...
    if dataset == 'icvl':
        ...
        names = ['Palm', 'Wrist1', 'Wrist2', 'Thumb.R', 'Thumb.T', 'Index.R', 'Index.T', 'Mid.R', 'Mid.T', 'Ring.R',
                 'Ring.T', 'Pinky.R', 'Pinky.T', 'Mean'];
    elif dataset == 'msra':
        joint_idx = [0, 1, 4, 5, 8, 9, 12, 13, 16, 17, 20, 21]
        names = ['Wrist', 'Index.R', 'Index.T', 'Mid.R', 'Mid.T', 'Ring.R', 'Ring.T', 'Pinky.R', 'Pinky.T', 'Thumb.R',
                 'Thumb.T', 'Mean']
...
def main():
    dataset = 'icvl'
    in_file = 'results/result_icvl_4x6x6.txt'
