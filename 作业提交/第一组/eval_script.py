...
def video2picture(video_path,picture_path,picture_amount):
    num = 0
    capture = cv2.VideoCapture(video_path)
    if (capture.isOpened()):
        print("video2picture processing")
    while True:
        flag, image = capture.read()
        if not flag:
            break
        image = cv2.resize(image, (256, 256))
        cv2.imwrite(picture_path + '{:0>5d}.jpg'.format(num), image)
        if num == picture_amount:
            break
        num = num + 1
    capture.release()
